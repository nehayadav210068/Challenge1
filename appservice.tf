...
resource "azurerm_linux_web_app" "fe-webapp" {
  ...
  app_settings = {
    ...
    "APPINSIGHTS_INSTRUMENTATIONKEY"                  = azurerm_application_insights.ch-appinsights.instrumentation_key
  ...
  depends_on = [
    azurerm_service_plan.fe-asp,azurerm_application_insights.ch-appinsights
  ]
}

#storage account for functionapp
resource "azurerm_storage_account" "fn-storageaccount" {
  name                     = "chfunctionappsa2023"
  resource_group_name      = azurerm_resource_group.rg.name
  location                 = azurerm_resource_group.rg.location
